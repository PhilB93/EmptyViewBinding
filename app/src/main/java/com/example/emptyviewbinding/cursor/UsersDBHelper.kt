...
class UsersDBHelper(context: Context) : SQLiteOpenHelper(context, DATABASE_NAME, null, DATABASE_VERSION) {
    ...
    fun insertUser(user: NbaPlayer): Boolean {
        ...
        values.put(DBContract.UserEntry.COLUMN_SKIN, user.skin)
    ...
    fun readUser(userid: Int): ArrayList<NbaPlayer> {
        val users = ArrayList<NbaPlayer>()
        val db = writableDatabase
        ...
        if (cursor!!.moveToFirst()) {
            while (cursor.isAfterLast == false) {
                ...
                age = cursor.getInt(cursor.getColumnIndex(DBContract.UserEntry.COLUMN_AGE))
                skin = cursor.getInt(cursor.getColumnIndex(DBContract.UserEntry.COLUMN_SKIN))

                users.add(NbaPlayer(userid, name, age, skin))
                cursor.moveToNext()
            }
        }
        return users
    ...
    fun readAllUsers(): ArrayList<NbaPlayer> {
        val users = ArrayList<NbaPlayer>()
        val db = writableDatabase
        var cursor: Cursor? = null
        try {
    ...
    companion object {
        ...
        val DATABASE_NAME = "players_table.db"
        ...
        private val SQL_CREATE_ENTRIES =
            "CREATE TABLE " + DBContract.UserEntry.TABLE_NAME + " (" +
                    ...
                    DBContract.UserEntry.COLUMN_SKIN + " TEXT)"
